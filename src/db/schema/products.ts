...
import { decimal, index, integer, json, text, timestamp, varchar } from 'drizzle-orm/pg-core'
...
export const products = pgTable(
  ...
  (table) => ({
    ...
    categoryIdIdx: index(`${dbPrefix}_products_category_id_idx`).on(table.categoryId),
    subcategoryIdIdx: index(`${dbPrefix}_products_subcategory_id_idx`).on(table.subcategoryId),
  })
)
