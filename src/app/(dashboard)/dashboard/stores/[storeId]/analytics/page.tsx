...
import { getCustomers, getOrderCount, getSaleCount, getSales } from '@/lib/actions/order'
...
export default async function AnalyticsPage({ params, searchParams }: AnalyticsPageProps) {
  ...
  const [saleCount, orderCount, sales, { customers, customerCount }] = await Promise.all([
    saleCountPromise,
    orderCountPromise,
    salesPromise,
    customersPromise,
      ...
      <div className='flex flex-col gap-4 2xl:flex-row'>
        <Card className='flex-1'>
          <CardHeader>
            ...
            <CardDescription>Total sales in the last {dayCount} days</CardDescription>
          </CardHeader>
          <CardContent>
            <SalesChart
              data={sales.map((sale) => ({
        ...
        <Card>
          ...
          <CardContent className='space-y-8'>
            {customers.map((customer) => (
              <div key={customer.email} className='flex flex-col gap-2 sm:flex-row sm:items-center'>
                <div className='flex flex-1 flex-col gap-4 sm:flex-row sm:items-center'>
                  ...
                  <div className='w-full space-y-1 text-sm'>
                    ...
                    <p className='break-all leading-none text-muted-foreground'>{customer.email}</p>
                  </div>
                </div>
              ...
              <PaginationContent>
                ...
                <PaginationItem>
                  <PaginationNext
                    ...
                    className={cn(
                      'transition-opacity',
                      Math.ceil(customerCount / 5) === page && 'pointer-events-none opacity-50'
                    )}
                  />
                </PaginationItem>
              </PaginationContent>
