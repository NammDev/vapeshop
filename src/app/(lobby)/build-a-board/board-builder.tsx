...
export function BoardBuilder({ products, pageCount, subcategory, cartItems }: BoardBuilderProps) {
  ...
  const addProductToCart = React.useCallback(
    async (product: Product) => {
      try {
        const hasProductInCart = cartItems.some((item) => item.productId === product.id)
      ...
      <div className='grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
        {products.map((product) => (
          <ProductCard
            ...
            variant='switchable'
            product={product}
            isAddedToCart={cartItems.map((item) => item.productId).includes(product.id)}
