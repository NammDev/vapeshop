...
export async function getCart(input?: { storeId: string }): Promise<CartLineItemSchema[]> {
  noStore()
...
export async function getUniqueStoreIds() {
  ...
  try {
    const cart = await db
      ...
      .from(carts)
      .leftJoin(products, sql`JSON_CONTAINS(carts.items, JSON_OBJECT('productId', products.id))`)
...
export async function deleteCartItem(input: z.infer<typeof deleteCartItemSchema>) {
  noStore()
...
export async function deleteCartItems(input: z.infer<typeof deleteCartItemsSchema>) {
  noStore()
