...
import { and, asc, countDistinct, desc, eq, gte, inArray, like, lte, sql } from 'drizzle-orm'
...
// export async function getStoreOrders(input: { storeId: string; searchParams: SearchParams }) {
//   noStore()
//   try {
//     const { page, per_page, sort, customer, status, from, to } = ordersSearchParamsSchema.parse(
//       input.searchParams
//     )
...
//             // Filter by status
//             statuses.length > 0 ? inArray(orders.stripePaymentIntentStatus, statuses) : undefined,
//             // Filter by createdAt
//             fromDay && toDay
...
export async function getOrderCount(input: { storeId: string; fromDay?: Date; toDay?: Date }) {
  noStore()
  try {
...
export async function getSaleCount(input: { storeId: string; fromDay?: Date; toDay?: Date }) {
  noStore()
  try {
    const { storeId, fromDay, toDay } = input
...
export async function getSales(input: { storeId: string; fromDay?: Date; toDay?: Date }) {
  noStore()
  try {
    const { storeId, fromDay, toDay } = input
