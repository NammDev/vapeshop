...
type ProductGroup = NonNullable<Awaited<ReturnType<typeof filterProducts>>['data']>[number]
    ...
    <>
      ...
      >
        ...
        <kbd className='pointer-events-none absolute right-1.5 top-2 hidden h-6 select-none items-center gap-1 rounded border bg-muted px-1.5 font-mono text-xs font-medium opacity-100 xl:flex'>
          <abbr title={isMac ? 'Command' : 'Control'} className='no-underline'>
            {isMac ? '⌘' : 'Ctrl'}
          </abbr>
          K
        </kbd>
      ...
      >
        ...
        <CommandList>
          ...
          {loading ? (
            ...
            data?.map((group) => (
              <CommandGroup key={group.name} className='capitalize' heading={group.name}>
                {group.products.map((item) => {
                  return (
                    <CommandItem
                      ...
                      className='h-9'
                      value={item.name}
                      onSelect={() => onSelect(() => router.push(`/product/${item.id}`))}
                    >
                      <Icons.product
