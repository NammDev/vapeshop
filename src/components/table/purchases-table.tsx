          ...
          return (
            <span>
              {safeParsedItems.success
                ? safeParsedItems.data.reduce((acc, item) => acc + item.quantity, 0)
                : 0}
            </span>
          )
        },
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align='end' className='w-[160px]'>
              <DropdownMenuItem asChild>
                <Link href={`/dashboard/purchases/${row.original.id}`}>Purchase details</Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild>
                <Link href={`/products?store_ids=${row.original.storeId}`}>View store</Link>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ),
